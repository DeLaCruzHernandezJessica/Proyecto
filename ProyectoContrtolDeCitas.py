...
def registrar_usuario():
 try:
     ...
     registro = Registrar(nombre, apellidop, apellidom, sexo, edad, telefono, cargo)
...
class Pacientes:
    def __init__(self, nombre, apellidop, apellidom, telefono, id_paciente=None):
        self.nombre=nombre
        self.apellidop=apellidop
        self.apellidom=apellidom
        self.telefono=telefono
...
def registro_paciente():
    try:
     ...
     apellidom=en_apellidom.get()
     telefono=en_telefono.get()
     if not all([nombre, apellidop, apellidom, telefono]):
         raise ValueError("Todos los campos son obligatorios")
     nuevo_paciente=Pacientes(nombre, apellidop, apellidom, telefono, id_paciente=None)
...
def formulario(var, indx, mode):
   global en_nombre, en_apellidom, en_apellidop, en_telefono, en_edad, en_cargo, en_sexo, id_paciente
   ...
   if seleccion=="Medico":
      ...
      en_telefono=tk.Entry(frame)
      en_telefono.grid(row=6, column=1, padx=10, pady=5)
      boton_enfermero=tk.Button(frame, text="Registrasr personal", command=registrar_usuario)
      ...
      boton_paciente.grid(row=5, column=0, columnspan=2, pady=10)
